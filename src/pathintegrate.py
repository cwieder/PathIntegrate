...
class PathIntegrate:
    ...
    def SingleViewCV(self, n_cv=5):
        # Set up sklearn pipeline

        pipe_sv = sklearn.pipeline.Pipeline([
            ('Scaler', StandardScaler()),
            ('sspa', self.sspa_method(self.pathway_source, self.min_coverage)),
            ('sv', sklearn.linear_model.LogisticRegression())
        ])
        cv_res = cross_val_score(pipe_sv, X=self.omics_data, y=self.labels, cv=5)

        return cv_res
    
    def SingleViewGridSearchCV(self):
        # Set up sklearn pipeline
        pipe_sv = sklearn.pipeline.Pipeline([
            ('Scaler', StandardScaler()),
            ('sspa', self.sspa_method(self.pathway_source, self.min_coverage)),
